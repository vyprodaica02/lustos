            ...
            <v-row>
              <v-col cols="6">
                ...
                <v-text-field
                  ...
                  variant="solo"
                  label="Họ tên"
                  v-model="editedUser.projectName"
                ></v-text-field>
              </v-col>
...
export default {
  ...
  methods: {
    ...
    ...mapActions(["projectModule"]), // Tạo method userModule tương đương với action userModule trong Vuex store
    async saveChanges() {
      ...
      await this.$store.dispatch("saveChangesActionProject", this.editedUser); // Sử dụng await để đảm bảo saveChangesAction hoàn thành trước khi gọi fetchUsers
      await this.$store.dispatch("fetchProject", this.inputValues);
      this.offModal();
    },
  },

...
export default {
  ...
  methods: {
    ...
    ...mapActions(["userModule"]), // Tạo method userModule tương đương với action userModule trong Vuex store
    async saveChanges() {
      ...
      await this.$store.dispatch("saveChangesAction", this.editedUser); // Sử dụng await để đảm bảo saveChangesAction hoàn thành trước khi gọi fetchUsers
      await this.$store.dispatch("fetchUsers", this.inputValues);
      this.offModal();
    },
  },

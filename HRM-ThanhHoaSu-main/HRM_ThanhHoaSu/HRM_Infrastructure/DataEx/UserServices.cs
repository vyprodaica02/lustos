...
{
    public class UserServices<T> : IRepository<T> where T : User
    {
        ...
        public void Add(T entity)
        {
            using (var trans = dbContext.Database.BeginTransaction())
            {
                try
                {
                    ...
                    if (Entities.Any(x => x.Email == entity.Email && x.Code == entity.Code))
                    {
                ...
                try
                {
                    if (entities.Any())
                    {
                        ...
                        else
                        {
                            ...
                            {
                                if (!_entities.Any(x => x.Email == entity.Email || x.Code == entity.Code))
                                {
        ...
        {
            var userHt = dbContext.qa_users.FirstOrDefault(x => x.Id == id);
